...
async function Edit(id) {
  let response = await api.Advogado.Edit(id);
...
async function Save() {
  try {
    await api.Advogado.Save(Model.value);
    Index();
  } catch {}
}
...
async function Delete() {
  try {
    let response = await api.Advogado.Delete(Model.value);
    Model.value = response.Dados;
    Index();
  } catch {}
}
